...
| `subsystems/kernels/qk-rope-smoke-oracles.md` | The hd256/hd512 QK-RoPE device gates keep three closed-form numerical anchors and derive every other expectation from a GPU arm an anchor already certifies (anchor → flat kernel → paged prefill → paged decode); no host-side second implementation of RMSNorm/RoPE/paged addressing. Records the gate command, the `PEGAINFER_REQUIRE_GPU=1` skip trap, and the negative controls. |
