...
- `hf_golden_gate.rs` picks the fixture by config CONTENT, never the directory name: it hashes the pointed model's `config.json` and scans `test_data/qwen3*-hf[-long]-golden.safetensors` for the fixture whose safetensors metadata records the same `config_sha256`. Exactly one match is required; zero matches is a clean skip with the dumper command, more than one is a panic. Every fixture also records `model_revision`, and `check_fixture_metadata` re-asserts both before a single logit is compared, so a mispairing fails an assert rather than comparing against the wrong oracle. Qwen3.8-27B reuses Qwen3.5-27B's geometry exactly, which is why the lookup keys on the config hash and not on a geometry table.
