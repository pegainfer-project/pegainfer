...
impl Qwen35Model {
    ...
    fn prefill_chunk_forward(
        ...
        let mut gdr_chunkwise_scratch =
            GdrChunkwiseScratch35::new(&self.ctx, c, self.tensor_parallel, seq_len)?;
    ...
    fn prefill_layer(
        ...
        let attn_out_dim = match &layer.attn {
            ...
            LayerKind::LinearAttention(_) => c.local_linear_z_dim(tp),
        };
    ...
    fn prefill_linear_attention(
        ...
        let qkv_dim = c.local_linear_qkv_dim(tp);
        let z_dim = c.local_linear_z_dim(tp);
        ...
        ops::gated_delta_rule_prefill_chunkwise_into(
            ...
            gdr_chunkwise_scratch,
            &mut gdr_out_batch,
            c.local_linear_num_key_heads(tp),
            c.local_linear_num_value_heads(tp),
