...
impl Qwen35TpExecutor {
    ...
    pub(crate) fn from_runtime_with_limits(
        ...
        let graph_enabled = enable_cuda_graph
            && first
                .config()
                .local_decode_group_is_compiled(TensorParallelConfig {
                    rank: 0,
                    world_size,
                });
        if enable_cuda_graph && !graph_enabled {
            ...
            LOG_GRAPH_GATE.call_once(|| {
                ...
                log::info!(
                    ...
                    config.local_num_attention_heads(TensorParallelConfig { rank: 0, world_size })
                        / config.local_num_key_value_heads(TensorParallelConfig { rank: 0, world_size }),
                    config.local_num_attention_heads(TensorParallelConfig { rank: 0, world_size }),
                    config.local_num_key_value_heads(TensorParallelConfig { rank: 0, world_size }),
                );
            });
