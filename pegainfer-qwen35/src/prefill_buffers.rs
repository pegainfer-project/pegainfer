...
impl GdrChunkwiseScratch35 {
    ...
    pub(crate) fn new(
        ...
        Self::from_dims(
            ctx,
            config.local_linear_num_value_heads(tp),
    ...
    pub(crate) fn estimate_bytes(
        ...
        let num_vh = config.local_linear_num_value_heads(tp);
        ...
        let intermediate = config.local_intermediate_size(tp);
        ...
        let full_qkv = config.local_full_attn_gated_q_dim(tp);
        let full_kv = config.local_full_attn_kv_dim(tp);
        let full_out = config.local_full_attn_q_dim(tp);
