...
impl EngineHarness {
    ...
    /// Latest scheduler occupancy snapshot. Pull-only — the step driver
    /// publishes into a cell, not a watch channel.
...
impl RequestStream {
    ...
    /// Non-blocking poll. `None` means nothing is queued yet (or the engine
    /// is gone); overlap tests use this to drain without waiting.
    pub(crate) fn try_recv(&mut self) -> Option<RequestUpdate> {
        let mut state = self.inbox.state.lock().unwrap();
        state
            .updates
            .get_mut(&self.id)
