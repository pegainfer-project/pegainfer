...
for command in git nvidia-smi nvcc rustc cargo protoc cc c++ clang cmake ninja \
  pkg-config rg sha256sum awk sed tee timeout realpath tr mktemp cp ar; do
...
http_request() {
  ...
  timeout --kill-after=10s 5m "$binary" --model-path "$model" --port "$port" --max-batch 8 \
...
mkdir -p "$log_root/binaries"
cp "$target_root/stock/release/pegainfer" "$log_root/binaries/stock"
cp "$CARGO_TARGET_DIR/release/pegainfer" "$log_root/binaries/candidate"
sha256sum "$log_root/binaries/stock" "$log_root/binaries/candidate" \
...
qwen35_test_binary="$("$python" - "$log_root/qwen35-tests-build.log" <<'PY'
import json, os, sys
executables = set()
with open(sys.argv[1]) as records:
    for line in records:
        ...
        if (record.get("reason") == "compiler-artifact"
                and record["target"]["name"] == "pegainfer_qwen35"
                and record["profile"]["test"] and record.get("executable")):
            executables.add(record["executable"])
assert len(executables) == 1, f"expected one Qwen3.5 test executable: {executables}"
executable = executables.pop()
assert os.path.isfile(executable) and os.access(executable, os.X_OK), executable
print(executable)
PY
)"
cp "$qwen35_test_binary" "$log_root/binaries/qwen35-tests"
sha256sum "$log_root/binaries/qwen35-tests" | tee "$log_root/qwen35-test-binary-sha256.log"
...
run_exact_gate() {
  ...
  if [[ "$label" == gate3-hf-golden ]]; then
    rg -Fx 'GDN_ACCEPTANCE_REJECTIONS_OK cases=7' "$log_root/$label.log"
  fi
  case "$label" in
...
run_exact_gate gate3-triton-hf \
...
# Same target, no clean: removing the bundle must remove its identity and link inputs.
env -u PEGAINFER_QWEN35_GDN_AOT_BUNDLE timeout 90m cargo build \
  --release --locked -p pegainfer-server --no-default-features --features qwen35 --bin pegainfer \
  --message-format=json-render-diagnostics 2>&1 | tee "$log_root/gate1-unlinked-rebuild.log"
"$python" - "$log_root/gate1-unlinked-rebuild.log" <<'PY_CHECK'
...
for line in Path(sys.argv[1]).read_text().splitlines():
    try:
        ...
        continue
    if record.get("reason") == "build-script-executed" and "pegainfer-kernels" in record["package_id"]:
        records.append(record)
assert len(records) == 1, f"expected one active kernels build, got {len(records)}"
record = records[0]
assert not any("cuda_dialect_runtime" in lib for lib in record["linked_libs"]), record
assert not any("qwen35-gdn-candidate" in path for path in record["linked_paths"]), record
archive = Path(record["out_dir"]) / "libkernels_cuda.a"
members = subprocess.check_output(["ar", "t", str(archive)], text=True).splitlines()
assert "kernel.o" not in members, f"stale candidate object in {archive}"
assert "qwen35_flashinfer_gdn_aot.o" in members, f"missing stock shim in {archive}"
print("CANDIDATE_REMOVAL_LINK_OK")
PY_CHECK
expect_startup_rejection gate1-unlinked-rebuild "$CARGO_TARGET_DIR/release/pegainfer" \
  "no AOT candidate was linked"
sha256sum -c "$log_root/binary-sha256.log"
sha256sum -c "$log_root/qwen35-test-binary-sha256.log"
