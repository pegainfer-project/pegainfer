...
//! `reasoning_effort` (default `xhigh`, restricted to `xhigh|medium|low`) and
//! gates the reasoning instructions on `enable_thinking`. Those knobs reach the
//! template through the renderer's `reasoning_effort` option and
//! `template_kwargs`. Frontend-owned for the same reason as
//! `gemma4_tokenizer_parity.rs`: only the render compares two implementations —
//! minijinja against the reference's Jinja2.
//!
