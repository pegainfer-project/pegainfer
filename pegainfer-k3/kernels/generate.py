...
projections are served by cuBLASLt and the routed experts by the DeepGEMM
masked grouped-GEMM chain, so no GEMV family is generated here either.
...
# the instantiation count and the per-block shared memory scale with it, so
# only the certification capacity is generated by default. Extending serving
# to longer contexts is this one-line change.
MAX_CTX = [128]
...
# Default thread count of the vendored kernels; `kda_core` and `mla_attn`
# override it (head_dim and 128 respectively).
THREADS = 256
...
MLA_ATTN_PARAMS = (
    f"(const {BF16}* __restrict__ Kc, const int* __restrict__ N, "
    f"{BF16}* __restrict__ O, const {BF16}* __restrict__ Q, "
    f"const {BF16}* __restrict__ Sc, const {BF16}* __restrict__ Vc)"
)
ROUTER_PARAMS = (
...
# to the block. None of the batched bodies do — their only bulk copy is
# `mla_attn`'s one-dimensional query load — so the launchers bind plain
# pointers and the requested thread count. That is an assumption about
# TileLang, not about the kernels, so it is asserted rather than trusted.
    ...
    for cap in MAX_CTX:
        for batch in B_BUCKETS:
            insts.append(Inst(
                family="mla_attn",
                order=len(insts),
                label=f"mla_attn_batched NH={MLA_HEADS} CAP={cap} B={batch}",
                factory="mla_attn_batched",
                args=(MLA_HEADS, QK_DIM, V_DIM, cap, batch, 128),
                num_params=6,
                params=MLA_ATTN_PARAMS,
                symbol=f"k3_mla_attn_b{batch}_nh{MLA_HEADS}_cap{cap}_kernel",
                grid=(batch, MLA_HEADS),
                threads=128,
                guard=(
                    f"b == {batch} && num_heads == {MLA_HEADS} && "
                    f"qk_dim == {QK_DIM} && v_dim == {V_DIM} && max_ctx == {cap}"
                ),
                call_args=(
                    _bf16("Kc"), "N", _bf16("O", False),
                    _bf16("Q"), _bf16("Sc"), _bf16("Vc"),
                ),
            ))
    return Plan(
        stem=_STEM.format("mla_attn"),
        signature=(
            "k3_mla_attn_batched(\n"
            "    const void* Q,\n"
            "    const void* Kc,\n"
        ...
        doc=(
            "// NoPE full-context attention, one (row, head) per block; every head\n"
    ...
    for blocks in ATTNRES_NB:
        for batch in B_BUCKETS:
            insts.append(Inst(
                ...
                args=(blocks, HIDDEN, batch, RMS_EPS, THREADS),
                num_params=4,
    ...
    for blocks in ATTNRES_NB:
        for batch in B_BUCKETS:
            insts.append(Inst(
                ...
                args=(blocks, HIDDEN, batch, THREADS),
                num_params=4,
                params=MIX_PARAMS,
