...
// complete routed-expert forward for a single rank (all routed experts local;
// the EP all-to-all is a later stage).
//
// ---------------------------------------------------------------------------
// Chain
// ---------------------------------------------------------------------------
...
//
//   topk_idx[entry]  local expert id, or anything outside [0, groups) to mark
//                    the entry inactive (padded batch rows, and later the
//                    non-local experts of an EP shard)
//   slot_map[entry]  `expert * masked_cap + rank` for active entries, -1 for
//                    inactive ones; `rank` counts the active entries of that
//                    expert that precede this one in entry order
//   masked_m[expert] number of active entries claiming that expert
//
...
__global__ void moe_local_route_metadata_kernel(const int* __restrict__ topk_idx,
  ...
  for (int chunk = 0; chunk < entries; chunk += kMetadataThreads) {
    ...
    const bool active = routed >= 0 && routed < groups;
    if (entry < entries && !active && entry % groups == expert) {
      slot_map[entry] = -1;
    }

    const int mine = active && routed == expert ? 1 : 0;
...
// Routing metadata for a single rank's local experts. `topk_idx` is
// `[tokens, topk]` i32; entries outside [0, groups) are inactive. Writes
// `masked_m[groups]` and `slot_map[tokens * topk]`.
...
CUresult k3_moe_gather_fp8_quant_masked_cuda(
    ...
    int groups, int masked_cap, cudaStream_t stream) {
  PEGAINFER_FFI_GUARD_BEGIN
  if (latent == nullptr || topk_idx == nullptr || slot_map == nullptr ||
      output == nullptr || scales == nullptr ||
